...
# set -v -x
#
# mk_yl.sh: generate a yb cluster in docker, interactive nodes (servers).
#         now use mk_nodesw.sh to generate sw components on new node..
#         additional: put flagfiles on persistent storage to survive upgrades
#
# todo:
#  - note: ??? background=true and ui=true are specified, but those values are default ?
#  - the docker run -d(etach) option was introduced on upgrade, is it neede ?
#  - reduce port-mappings ? striktly, we only need 5433, 7000, 9000 and 15433 
#     remove 12xxx and 13xxx
#
# done..
...
# docker exec node2 yugabyted start --advertise_address=node2 --background=true --ui=true
  ...
  startcmd=`echo docker exec node2 yugabyted start \
    ...
    --tserver_flags=flagfile=/root/var/conf/yb_tsrv_flags.conf \
     --master_flags=flagfile=/root/var/conf/yb_mast_flags.conf `
